...
void print_to_consol_cplx_x(cplx*);             //my test
void print_to_consol_int(int*);                 //my test
void my_test(void);// my test

typedef enum {
	depressed, pressed, long_pressed, hold
}  button_state_t;
...
typedef enum {
             off,
             on
             } state_toggle;                       // my test

typedef enum {
	          top_button,
	          central_button,
			  bottom_button
             } button_position;                 // my test

typedef struct {                             // my test
	uint64_t current_time;
	uint64_t previous_time;
	unsigned char counter_click;
	uint64_t counter_unclick;
} my_time;
